...
QString Utils::Encodebs64(const QByteArray &binaryData) throw()
{
    ...
    {   // 将3字节数据转换成4个ASCII字符
        ...
        else
        {
            int b0 = binaryData[index++] & 0xFF;
            int b1;
            i == 2 ? b1 = (binaryData[index++] & 0xFF) : b1 = 0;
            sb.append(DATA_BIN2ASCII[b0 >> 2]);
            sb.append(DATA_BIN2ASCII[((b0 << 4) | (b1 >> 4)) & 0x3F]);
            i == 1 ? sb.append('=') : sb.append(DATA_BIN2ASCII[(b1 << 2) & 0x3F]);
            sb.append('=');     // 填充字符
        }
    }
    return sb;
...
QByteArray Utils::Decodebs64(const QString &base64String) throw(int)
{
...
qint64 Utils::AtoInt64(char *a)
{
    const int length = 8;
    qint64 r = 0;
    for(int i = 0; i < length; i++) {
        qint64 un = qint64(quint8(a[i]));
        r += un << (length - 1 - i)*8;
    }
    return r;
}
